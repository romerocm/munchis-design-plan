...
import { parseLocalDate, formatDay, formatDayOffset } from "@/lib/format";
...
function buildSteps(drop: Drop, orderedCount: number): TimelineStep[] {
  ...
  const pickupActive = drop.status === "ready";
  ...
  return [
    ...
    },
    {
      ...
      detail: `${bakingDay} · ${orderedCount * 4} treats, handmade`,
    ...
    },
    {
      ...
      state: (pickupDone ? "done" : pickupActive ? "active" : "upcoming") as StepState,
    },
  ];
}
        ...
        return (
          <div key={i} className={`flex items-start gap-4 ${isLast ? "" : "pb-6"}`}>
            ...
            <div className="flex flex-col gap-0.5 pt-1">
              ...
              <span className={`text-[13px] ${step.state === "upcoming" ? "text-forest/25" : "text-forest/40"}`}>
                {step.detail}
              </span>
            </div>
          </div>
