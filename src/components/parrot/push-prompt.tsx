...
export function PushPrompt() {
  ...
  // Hide when subscribed or not supported
  if (!isSupported || subscribed || dismissed) return null;

  // Show blocked message if user denied
  if (permission === "denied") {
    return (
      <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-red-soft border border-red-200">
        <p className="text-xs text-red-text">Notifications blocked — enable in your device settings to get order alerts.</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-2">
      <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-forest/5 border border-forest/8">
        ...
        <button
          ...
          disabled={loading}
          className="px-3 py-1.5 rounded-lg bg-forest text-white text-xs font-semibold flex-shrink-0 btn-press disabled:opacity-50"
        >
          {loading ? "..." : "Enable"}
        </button>
        <button
      ...
      {error && (
        <p className="text-xs text-red-text px-4">Something went wrong. Check console for details.</p>
      )}
    </div>
  );
}
